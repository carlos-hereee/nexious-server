...
export const getAllUserTaskBoard = async (req: AppRequest, res: Response) => {
  try {
    const target = req.user.boards?.map((board) => board.boardId);
    if (target) {
      const boards = await BoardTasks.find({ _id: { $in: target } });
      if (boards && boards.length > 0) return res.status(200).json(boards).end();
    }
