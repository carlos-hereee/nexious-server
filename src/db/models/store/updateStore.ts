...
export const updateStore = async (params: UpdateStoreParams) => {
  const { accountId, type, order, stripe, orderId, status, storeId, paymentStatus } = params;
  ...
  if (type === "checkout-complete-paid" && orderId && status && paymentStatus) {
    return await Store.updateOne(
      { accountId, "orders.orderId": orderId },
      // update order status and merchandise payment status
      { $set: { "orders.$.status": status, "orders.$.merch.$[m].paymentStatus": paymentStatus } },
